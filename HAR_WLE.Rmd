...
![](test_df_scatter_roll_belt_magnet_dumbbell_z_save.png)
![](test_df_scatter_magnet_dumbbell_z_pitch_forearm_save.png)
...
Very low (compared to other machine learning exercises) since the random forests generated near-zero prediction mistakes on the validation data set. Prediction accuracy on testing data was `0.9837`. Therefore, for 100 new observations the model is expected to make atmost ``r round((1 - 0.9837) * 100.0, 0)`` errors. 
...
The prediction accuracy has marginally increased from `0.9451` (sampled training data fit predicted on validation data) to `0.9837` (all training & validation data fit predicted on testing {holdout sample} data). There's some minor re-ordering of variable importances (e.g. magnet_dumbbell_x moves higher than pitch_forearm).
...
png(filename="test_df_scatter_roll_belt_magnet_dumbbell_z.png")
#print(ggplot(test_plot_df, aes_string(x=feats_top_importance_lst[1], 
#                                      y=feats_top_importance_lst[2])) + 
print(ggplot(test_plot_df, aes_string(x="roll_belt", 
                                      y="magnet_dumbbell_z")) + 
...
png(filename="test_df_scatter_magnet_dumbbell_z_pitch_forearm.png")
#print(ggplot(test_plot_df, aes_string(x=feats_top_importance_lst[2], 
#                                      y=feats_top_importance_lst[3])) + 
print(ggplot(test_plot_df, aes_string(x="magnet_dumbbell_z", 
                                      y="pitch_forearm")) + 
          geom_point(aes_string(color=response_varname, 
                              shape="shape_error"), position="jitter") +  
        scale_color_brewer(type="qual", palette="Set1") +  
...
feats_plot_df <- mutate(feats_plot_df, 
                        feature_tmp = ifelse((variable == "sel_mdl.Importance"), feature, ""))
