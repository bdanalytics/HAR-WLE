...
2. Optimized fitting the model (first run on training data took a long time & did not complete):  
    2.1 Partitioned training data into training (60%), validation (20%) & testing (20%) data 
...
Very low (compared to other machine learning exercises) since the random forests generated near-zero prediction mistakes on the validation data set. Prediction accuracy on testing data was `0.9837`. Therefore, for 100 new observations the model is expected to make ``r round((1 - 0.9837) * 100.0, 0)`` errors. 
...
1. Plot the decision boundaries in the final model (top 2-3 random forest importance variables)
...
print(ggplot(train_df, aes_string(x=feats_tmp_df[1, "feature"], 
                                   y=feats_tmp_df[2, "feature"],
                                   color=response_varname)) + 
          geom_point())
print(ggplot(train_df, aes_string(x=feats_tmp_df[1, "feature"], 
                                   y=feats_tmp_df[3, "feature"],
                                   color=response_varname)) + 
          geom_point())
