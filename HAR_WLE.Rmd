...
train_df <- entity_df[inTrain, c(feats_df$feature, response_varname, id_varnames)]
test_df <- entity_df[-inTrain, c(feats_df$feature, response_varname, id_varnames)]

inValidate <- createDataPartition(y=train_df[, response_varname], p=0.8, list=FALSE)
train_df <- train_df[inValidate, c(feats_df$feature, response_varname, id_varnames)]
validate_df <- train_df[-inValidate, c(feats_df$feature, response_varname, id_varnames)]
...
#### Step ``r script_df[script_df$chunk_label == "partition_data", "chunk_step_major"]``: partition data
```{r partition_data}
```
#### Step ``r script_df[script_df$chunk_label == "inspect_features", "chunk_step_major"]``: inspect features
```{r inspect_features}
```
