...
-7. The rows with calculated aggregate measures (identified by the variable new_window=“yes”) shd be removed
-6. Plot text labels at 45 degrees
-5. roll_belt seems to be the key predictor for classe 'A' vs. others
-4. check decision boundaries for roll_arm vs. roll_forearm
-3. Feature distribution checks
-2. Check which new_df features are not in the ranges of entity_df
-1. Change predict_df to new_df; Swap names between validation & test
-1. Display one tree of the random forest ?
...
feats_df <- mutate(feats_df, label = ifelse(((nzv) | 
                                            (freqRatio >=25) | 
                                            (percentUnique <= 10)), feature, ""))

# Convert these to shiny interactive plots
print(ggplot(feats_df, aes(x=percentUnique, y=freqRatio)) + 
          geom_point(aes(color=nzv)) + 
          geom_text(aes(label=label), color="slategrey", size=3.5))

nearZeroVar_plot_df <- rbind(mutate(entity_df[, c("new_window", response_varname)], 
                                    source=factor("training", levels=c("training", "newdata"))),
                             mutate(predict_df[, "new_window", FALSE], 
                                    source=factor("newdata", levels=c("training", "newdata")),
                                    classe="OOB"))
myplot_histogram(nearZeroVar_plot_df, "new_window", 
                 fill_col_name=response_varname, facet_frmla=reformulate("source"))

```

Since new_window is a categorical variable, let's see if "yes" obs contains data that is different from the others.

#### Step ``r script_df[nrow(script_df), "chunk_step_major"]``: inspect features
```{r inspect_features_1_1, cache=TRUE}

entity_stats_df <- rbind(cbind(summaryBy(. ~ new_window, 
                                         data=entity_df[, feats_df$feature], 
                                         FUN=min, keep.names=TRUE),
                                data.frame(type="min", stringsAsFactors=FALSE)),
                         cbind(summaryBy(. ~ new_window, 
                                         data=entity_df[, feats_df$feature], 
                                         FUN=max, keep.names=TRUE),
                                data.frame(type="max", stringsAsFactors=FALSE)))
print(head(entity_stats_df))
entity_stats_tmp_df <- entity_stats_df
entity_stats_df <- melt(entity_stats_tmp_df, id.vars=c("new_window", "type"))
print(head(entity_stats_df))

entity_stats_tmp_df <- entity_stats_df
entity_stats_df <- dcast(entity_stats_tmp_df, variable ~ type + new_window)
entity_stats_df <- mutate(entity_stats_df, 
    is.range.overlap=ifelse((((max_yes >= max_no) & (min_yes <= min_no)) | 
                             ((max_yes <= max_no) & (min_yes >= min_no))) 
                            , FALSE, TRUE))
print(head(subset(entity_stats_df, is.range.overlap)))
#myplot_violin(entity_df, entity_stats_df[entity_stats_df$is.range.overlap, "variable"], "new_window")

myplot_violin(entity_df, 
              toString(entity_stats_df[entity_stats_df$is.range.overlap, "variable"][1]), 
              "new_window")
myplot_violin(entity_df, 
              toString(entity_stats_df[entity_stats_df$is.range.overlap, "variable"][2]), 
              "new_window")
myplot_violin(entity_df, 
              toString(entity_stats_df[entity_stats_df$is.range.overlap, "variable"][3]), 
              "new_window")

entity_plot_df <- melt(entity_df, id.vars=c("new_window"), 
                       measure.vars=entity_stats_df[entity_stats_df$is.range.overlap, "variable"])
print(head(entity_plot_df))
entity_plot_medians_df <- summaryBy(value ~ variable , entity_plot_df, FUN=c(median), na.rm=TRUE)
p <- ggplot(entity_plot_df, aes(x=new_window, y=value))
p <- p + xlab(" ")
...
p <- p + geom_text(data=entity_plot_medians_df,
...
print(p)
#myplot_violin(df=entity_plot_df, 
#    ycol_names=entity_stats_df[entity_stats_df$is.range.overlap, "variable"], 
#   xcol_name="new_window", facet_spec=NULL)
# mltd_df <- entity_plot_df


all.equal(entity_stats_save_df, entity_stats_df)

entity_stats_max_df <- summaryBy(. ~ new_window, data=entity_df[, feats_df$feature], 
                                 FUN=max, keep.names=TRUE)
entity_stats_max_df$type <- "max"
# entity_stats_plot_df <- melt(rbind(entity_stats_min_df, entity_stats_max_df), 
#                              id.vars=c("new_window", "type"))
entity_stats_df <- rbind(entity_stats_min_df, entity_stats_max_df) 
#entity_stats_df <- orderBy(~ type, entity_stats_df)
print(head(entity_stats_df))
entity_stats_save_df <- entity_stats_df

# myplot_violin(entity_df, "roll_belt", "new_window")
# myplot_violin(entity_df, "pitch_belt", "new_window")
# myplot_violin(entity_df, "yaw_belt", "new_window")
# myplot_violin(entity_df, "total_accel_belt", "new_window")

myplot_violin(entity_df, "num_window", "new_window")

entity_stats_min_df <- summaryBy(. ~ new_window, data=entity_df[, feats_df$feature], 
                                 FUN=min, keep.names=TRUE)
entity_stats_min_df$type <- "min"
entity_stats_max_df <- summaryBy(. ~ new_window, data=entity_df[, feats_df$feature], 
                                 FUN=max, keep.names=TRUE)
entity_stats_max_df$type <- "max"
# entity_stats_plot_df <- melt(rbind(entity_stats_min_df, entity_stats_max_df), 
...
require(fmsb)
# -2 for new_window & type
print(sprintf("Total feats in entity_stats_save_df: %d", ncol(entity_stats_save_df) - 2))    
feats_tmp_lst <- sort(names(entity_stats_save_df))
feats_tmp_lst <- feats_tmp_lst[!(feats_tmp_lst %in% c("new_window", "type", "num_window"))]
print("feats:")
print(feats_tmp_lst)
entity_stats_tmp2_df <- subset(entity_stats_save_df, (type == "max")
#                             , select=c("new_window", "type", 
#                                     grep(
#                                         "^(magnet|accel|gyros)_", 
#                                         #"^magnet_", 
#                                         #"^magnet_(belt|arm|dumbbell)_", 
#                                         names(entity_stats_save_df), value=TRUE)
#                                         )
                )

op <- par(mar=c(1, 2, 2, 1),mfrow=c(4, 2))
entity_stats_tmp2_df <- subset(entity_stats_save_df, (type == "max")
                            , select=c("new_window", "type", 
                                       feats_tmp_lst[1:12]))
#print(entity_stats_tmp2_df)
radarchart(subset(entity_stats_tmp2_df, select=-c(new_window, type)), 
           axistype=2, maxmin=FALSE, vlcex=0.75, palcex=0.75)
...
##where seg is the individual instance identifier -> changed to new_window
##cluster is the cluster membership -> changed to type
...
radarFix = function(d){
  ...
  d$new_window=as.factor(d$new_window)
  ##find increment
  angles = seq(from=0, to=2*pi, by=(2*pi)/(ncol(d)-2))
  ##create graph data frame
  graphData= data.frame(new_window="", x=0,y=0)
  graphData=graphData[-1,]

  for(i in levels(d$new_window)){
    new_windowData= subset(d, new_window==i)
    for(j in c(2:(ncol(d)-1))){
      ##set minimum value such that it occurs at 0. (center the data at -3 sd)
      new_windowData[,j]= new_windowData[,j]+3

      graphData=rbind(graphData, data.frame(new_window=i, 
                                            x=new_windowData[,j]*cos(angles[j-1]),
                                            y=new_windowData[,j]*sin(angles[j-1])))
    }
    ##completes the connection
    graphData=rbind(graphData, data.frame(new_window=i, 
                                          x=new_windowData[,2]*cos(angles[1]),
                                          y=new_windowData[,2]*sin(angles[1])))

  }
  graphData

}
radarData = ddply(entity_stats_df[, c("new_window", "type", 
                                      "roll_belt", "pitch_belt", "yaw_belt", "total_accel_belt")], .(type), radarFix)
ggplot(radarData, aes(x=x, y=y, group=new_window))+
  geom_path(alpha=0.5,colour="black")+
  geom_point(alpha=0.2, colour="blue")+
  facet_wrap(~type)
#### stackoverflow code ends here

print(ggplot(entity_stats_plot_df[entity_stats_plot_df$variable %in% feats_df$feature[3:4], ], aes(x=variable, y=value)) + 
                 geom_point(aes(color=new_window, shape=type)))

accel_feats <- grep("accel", feats_df$feature, value=TRUE)
print(head(entity_df[entity_df$new_window == "yes", c("num_window", "new_window", accel_feats)]))
print(head(entity_df[entity_df$new_window == "no", c("num_window", "new_window", accel_feats)]))



entity_stats_df <- summaryBy(. ~ new_window, data=entity_df[, feats_df$feature], 
                             FUN=c(min, max))
entity_stats_plot_df <- melt(entity_stats_df, id.vars="new_window")
entity_stats_plot_df$variable <- strsplit(toString(entity_stats_plot_df$variable), ',')[[1]]
entity_stats_plot_df$feature <- sapply(entity_stats_plot_df$variable, 
                                       function(variable) 
                                           strsplit(variable, ".", fixed=TRUE)[[1]][1])
entity_stats_plot_df$type    <- sapply(entity_stats_plot_df$variable, 
                                       function(variable) 
                                           strsplit(variable, ".", fixed=TRUE)[[1]][2])

entity_stats_dcast_df <- dcast(entity_stats_plot_df, feature ~ type, mean)
print(ggplot(entity_stats_plot_df, aes(x=)))
...
# freqRatio higher than nearZeroVar
hi_freqRatio_plot_df <- melt(entity_df[, c(response_varname, 
                                           "pitch_arm", "pitch_forearm", "roll_arm")], 
                             id.vars=response_varname)
myplot_histogram(hi_freqRatio_plot_df, "value", fill_col_name=response_varname,
                 facet_frmla=reformulate("variable"))

# percentUnique lower than nearZeroVar (least values filter)
# print(ggplot(feats_df, aes(x=percentUnique, y=freqRatio)) + 
#           geom_point(aes(color=nzv)) + xlim(-2, 10) + ylim(0.95, 1.15) + 
#           geom_text(aes(label=label), color="slategrey", size=3.5))

print(subset(feats_df, percentUnique <= 0.4))
lo_percentUnique_plot_df <- melt(entity_df[, c(response_varname, 
                                           "total_accel_belt", "total_accel_dumbbell")], 
                             id.vars=response_varname)
myplot_histogram(lo_percentUnique_plot_df, "value", fill_col_name=response_varname,
                 facet_frmla=reformulate("variable"))
...
mycompute_correlations <- function(feats_df, data_df, tgt_colname, min.cor_colname) {
    cor_varname <- paste0(tgt_colname, ".cor")
    cor_abs_varname <- paste0(cor_varname, ".abs")
    
    feats_cor_df <- data.frame(cor=cor(data_df[, feats_df$feature], 
                                       unclass(data_df[, tgt_colname])))
    names(feats_cor_df) <- cor_varname
    feats_cor_df[, cor_abs_varname] <- abs(feats_cor_df[, cor_varname])
    ...
    feats_ret_df <- feats_tmp_df[order(feats_tmp_df[, cor_abs_varname], decreasing=TRUE), ]

    #feats_all_df$freqRatioCut_fctr <- cut(feats_all_df$freqRatio, 5)
    print(ggplot(feats_ret_df, aes_string(x="percentUnique", y=cor_varname)) + 
            geom_point() + 
            geom_text(aes_string(label="feature", color="freqRatio", size=cor_abs_varname)))

    drop_feats <- feats_ret_df$feature[which(feats_ret_df[, cor_abs_varname] < 
                        feats_ret_df[feats_ret_df$feature == min.cor_colname, cor_abs_varname])]
    if (length(drop_feats) > 0) {
        print(sprintf("Dropping features based on %s correlation: %s", 
                      min.cor_colname, drop_feats))
        feats_ret_df <- feats_ret_df[!(feats_ret_df$feature %in% drop_feats),]
    }
        
    return(feats_ret_df)
}
feats_df <- mycompute_correlations(feats_df, data_df=train_df, 
                tgt_colname=response_varname, min.cor_colname=random_varname)

# Select features that have higher correlation than the random variable that was inducted ('my.rnorm')
