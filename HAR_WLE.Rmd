...
mymerge_varimp <- function(feats_df, mdl, mdl_name) {
    ...
    intersect(names(varimp_df), names(feats_df))
    
    feats_df <- merge(feats_df, varimp_df, all.x=TRUE)
    print(ggplot(feats_df, aes_string(x="response.cor", y=varimp_varname)) +
            geom_point(aes(size=freqRatio, shape=percentUniqueCut_fctr), color="grey") +   
            geom_text(aes(label=feature), size=3.5, color="blue"))
    return(feats_df)
}
